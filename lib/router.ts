...
export async function runCodingAgent({ task, mode, existingCode, image }: {
  ...
  /* ── Layer 4: Gemini Final Review & Bug-Fix Pass ──────────────── */
  /*
   * After all stages complete, Gemini reviews the generated code for
   * real-world bugs that open-source models commonly introduce:
   *   - CSS contrast failures (dark text on dark backgrounds)
   *   - JS state machine bugs (NaN, empty strings, missing listeners)
   *   - HTML structure issues (duplicate DOCTYPEs, broken semantics)
   *   - Placeholder images that won't render in production
   *
   * If bugs are found, Gemini returns the complete fixed code which
   * replaces the build artifact before the user ever sees it.
   */
  if (hasGemini) {
...
/* ─── Gemini Final Review & Bug-Fix Layer ───────────────────────────────── */
